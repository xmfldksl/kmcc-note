...
- 설명투(~합니다, ~했다, ~된다, ~이다)를 쓰지 않고 명사형 줄임체로 끝맺는다(예: 의결, 공개, 부과, 추진, 확대)
...
3. 설명투(~합니다, ~했다, ~된다, ~이다)를 쓰지 않고 명사형 줄임체로 끝맺는다(예: 의결, 보고, 부과, 추진).
4. 번호는 목록형 문법이 아니라 실제 숫자와 마침표(1. 2. 3.)로 적는다.
5. 문서가 여러 개면 각 문서를 【문서명】 제목 아래 구분해 요약한다.
6. 문서가 특정 차수의 회의(의사일정, 회의록, 속기록, N차 위원회 등) 관련이면:
...
7. 문서가 특정 차수의 회의와 관련이 없으면:
   숫자와 마침표(1. 2.)로 항목을 나누고, 둘째 깊이는 원문자 번호(① ② ③),
   셋째 깊이부터는 "- "로 시작해 정리한다. 결과에 관한 내용은 반드시 포함한다.
8. [cite: 숫자] 형태의 출처 표기는 모두 삭제한다.
9. 인사말, 맺음말, 부가 설명 없이 요약 본문만 출력한다.
...
def summarize(item):
    ...
    - 전체요약·피드요약 모두 명사형 줄임체(개조식)로 통일한다.
